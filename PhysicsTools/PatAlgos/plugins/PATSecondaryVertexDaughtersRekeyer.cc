...
namespace pat {
  class PATSecondaryVertexDaughtersRekeyer : public edm::stream::EDProducer<> {
  public:
    explicit PATSecondaryVertexDaughtersRekeyer(const edm::ParameterSet &iConfig);
    ~PATSecondaryVertexDaughtersRekeyer() override;
...
PATSecondaryVertexDaughtersRekeyer::PATSecondaryVertexDaughtersRekeyer(const edm::ParameterSet &iConfig):
...
PATSecondaryVertexDaughtersRekeyer::~PATSecondaryVertexDaughtersRekeyer() {}

void PATSecondaryVertexDaughtersRekeyer::produce(edm::Event &iEvent, edm::EventSetup const &) {
  ...
  for (size_t i = 0; i < src->size(); ++i) {
    ...
    for (const reco::CandidatePtr &p : daughters) {
      //
      ...
      //
      if (p.id() == pcOriHandle.id()){
        outPtrP->back().addDaughter(edm::Ptr<reco::Candidate>(pcNewHandle,p.key()));
      }else{
        outPtrP->back().addDaughter(p);
      }
    }
  }
...
DEFINE_FWK_MODULE(PATSecondaryVertexDaughtersRekeyer);