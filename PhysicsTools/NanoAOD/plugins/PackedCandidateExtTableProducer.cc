...
void PackedCandidateExtTableProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  std::vector<float> PFCand_pS1Energy;  PFCand_pS1Energy.reserve(pfCands->size());
  std::vector<float> PFCand_pS2Energy;  PFCand_pS2Energy.reserve(pfCands->size());

  std::vector<int> PFCand_blockIdx; PFCand_blockIdx.reserve(pfCands->size());

  std::vector<int> PFCand_key; PFCand_key.reserve(pfCands->size());
  ...
  for (size_t iPtr = 0; iPtr < candPtrs.size(); ++iPtr) {
    ...
    PFCand_key.push_back(candPtr.key());
    ...
    PFCand_pS1Energy.push_back(pfref.get()->pS1Energy());
    PFCand_pS2Energy.push_back(pfref.get()->pS2Energy());
  ...
  candTable->addColumn<float>("pS1Energy",   PFCand_pS1Energy,   "pS1Energy",   -1);
  candTable->addColumn<float>("pS2Energy",   PFCand_pS2Energy,   "pS2Energy",   -1);

  candTable->addColumn<int>("key",        PFCand_key,         "key");
  candTable->addColumn<int>("blockIdx",         PFCand_blockIdx,         "blockIdx");
