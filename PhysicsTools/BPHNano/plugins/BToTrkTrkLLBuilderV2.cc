...
class BToTrkTrkLLBuilderV2 : public edm::global::EDProducer<> {
  // perhaps we need better structure here (begin run etc)
public:
  typedef std::vector<reco::TransientTrack> TransientTrackCollection;

  explicit BToTrkTrkLLBuilderV2(const edm::ParameterSet &cfg)
...
void BToTrkTrkLLBuilderV2::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &iSetup) const {
  // input
  ...
  for (size_t ditracks_idx = 0; ditracks_idx < ditracks->size(); ++ditracks_idx) {
    ...
    int trk1_isPacked  = ditracks_ptr->userInt("trk1_isPacked");
    int trk2_isPacked  = ditracks_ptr->userInt("trk2_isPacked");
    int trk1_isLostTrk = ditracks_ptr->userInt("trk1_isLostTrk");
    int trk2_isLostTrk = ditracks_ptr->userInt("trk2_isLostTrk");
    int trk1_keyPacked = ditracks_ptr->userInt("trk1_keyPacked");
    int trk2_keyPacked = ditracks_ptr->userInt("trk2_keyPacked");
    ...
    for (size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
      ...
      cand.addUserInt("trk1_isPacked",  trk1_isPacked);
      cand.addUserInt("trk2_isPacked",  trk2_isPacked);
      cand.addUserInt("trk1_isLostTrk", trk1_isLostTrk);
      cand.addUserInt("trk2_isLostTrk", trk2_isLostTrk);
      cand.addUserInt("trk1_keyPacked", trk1_keyPacked);
      cand.addUserInt("trk2_keyPacked", trk2_keyPacked);
      ...
      if (trk1_isPacked){
        ptr_out->push_back(PackedCandidatePtr(packedPFCandidates,trk1_keyPacked));
      }
      if (trk2_isPacked){
        ptr_out->push_back(PackedCandidatePtr(packedPFCandidates,trk2_keyPacked));
      }
...
DEFINE_FWK_MODULE(BToTrkTrkLLBuilderV2);
