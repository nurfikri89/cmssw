...
class DiTrackBuilderV2 : public edm::global::EDProducer<> {
public:
  typedef std::vector<reco::TransientTrack> TransientTrackCollection;

  explicit DiTrackBuilderV2(const edm::ParameterSet &cfg)
...
void DiTrackBuilderV2::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &) const {
  // inputs
  ...
  for (size_t trk1_idx = 0; trk1_idx < pfcands->size(); ++trk1_idx) {
    ...
    for (size_t trk2_idx = trk1_idx + 1; trk2_idx < pfcands->size(); ++trk2_idx) {
      ...
      // ispacked or is lostTrk
      ditrack_cand.addUserInt("trk1_isPacked", trk1_ptr->userInt("isPacked"));
      ditrack_cand.addUserInt("trk2_isPacked", trk2_ptr->userInt("isPacked"));
      ditrack_cand.addUserInt("trk1_isLostTrk", trk1_ptr->userInt("isLostTrk"));
      ditrack_cand.addUserInt("trk2_isLostTrk", trk2_ptr->userInt("isLostTrk"));
      if(trk1_ptr->userInt("isPacked")){
        ditrack_cand.addUserInt("trk1_keyPacked", trk1_ptr->userInt("keyPacked"));
      }else{
        ditrack_cand.addUserInt("trk1_keyPacked", -1);
      }
      if(trk2_ptr->userInt("isPacked")){
        ditrack_cand.addUserInt("trk2_keyPacked", trk2_ptr->userInt("keyPacked"));
      }else{
        ditrack_cand.addUserInt("trk2_keyPacked", -1);
      }
      // save cands
      ...
      if (trk1_ptr->userInt("isPacked")){
        ptr_out->push_back(PackedCandidatePtr(packedPFCandidates,trk1_ptr->userInt("keyPacked")));
      }
      if (trk2_ptr->userInt("isPacked")){
        ptr_out->push_back(PackedCandidatePtr(packedPFCandidates,trk2_ptr->userInt("keyPacked")));
      }
...
DEFINE_FWK_MODULE(DiTrackBuilderV2);
