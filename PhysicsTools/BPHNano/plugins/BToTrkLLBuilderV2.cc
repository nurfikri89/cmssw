...
class BToTrkLLBuilderV2 : public edm::global::EDProducer<> {
  // perhaps we need better structure here (begin run etc)
public:
  typedef std::vector<reco::TransientTrack> TransientTrackCollection;

  explicit BToTrkLLBuilderV2(const edm::ParameterSet &cfg)
...
void BToTrkLLBuilderV2::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &iSetup) const {
  // input
  ...
  for (size_t k_idx = 0; k_idx < kaons->size(); ++k_idx) {
    ...
    for (size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
      ...
      cand.addUserInt("trk_isPacked",  k_ptr->userInt("isPacked"));
      cand.addUserInt("trk_isLostTrk", k_ptr->userInt("isLostTrk"));
      if(k_ptr->userInt("isPacked")){
        cand.addUserInt("trk_keyPacked", k_ptr->userInt("keyPacked"));
      }else{
        cand.addUserInt("trk_keyPacked", -1);
      }
      ...
      if (k_ptr->userInt("isPacked")){
        ptr_out->push_back(PackedCandidatePtr(packedPFCandidates,k_ptr->userInt("keyPacked")));
      }
    }  // for(size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
  }  // for(size_t k_idx = 0; k_idx < kaons->size(); ++k_idx)
...
DEFINE_FWK_MODULE(BToTrkLLBuilderV2);
